...
public class AdminMemberControllerImpl extends BaseController implements AdminMemberController{
	...
	public ModelAndView adminGoodsMain(@RequestParam Map<String, String> dateMap, HttpServletRequest request,
		...
		if(beginDate == null && endDate == null) {
			initDate = calcInitPeriod().split(",");
			beginDate = initDate[0];
			endDate = initDate[1];
		}
	...
	public ModelAndView memberDetail(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		MemberVO member_info = null;
		if(member_id!=null) {
			member_info = adminMemberService.memberDetail(member_id);
		}else {
			member_info = adminMemberService.memberDetail(search_word);
		}
