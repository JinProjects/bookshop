	...
	<form name="post">
		<table>
			<tbody>
				<tr>
					<td>
						<input type="radio" name="simple" checked="checked">간단조회 &nbsp;&nbsp;&nbsp;
										...
										<strong>
											<c:forEach var="item2" items="${myOrderHistList}" varStatus="j">
												<c:if test="${item.order_id == item2.order_id }">
													<a href="${contextPath }/goods/goodsDetail.do?goods_id=${item2.goods_id}">${item2.goods_title }</a>
												</c:if>
											</c:forEach>
										</strong>
									</td>
									...
									<td>
										<strong>
											<c:choose>
												...
												<c:when test="${item.delivery_state=='calcel_order' }">
													주문취소
												</c:when>
