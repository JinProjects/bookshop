...
<c:set var="orderList" value="${orderMap.orderList }" /> <!-- 주문상품리스트 -->
<c:set var="deliveryInfo" value="${orderMap.deliveryInfo }" /> <!-- 배송정보 -->
<c:set var="orderer" value="${orderMap.orderer }"/> <!-- 주문자정보 -->
	...
	<table class="list_view">
		...
		<tr>
			<c:forEach var="item" items="${orderList }">
				...
				<td>
					<h2>
						<a href="${contextPath }/goods/goodsDetail.do?goods_id=${goods_id}">${item.goods_title }</a>
					</h2>
				</td>
				<td>
				...
				<td>
					<h2>${1500*item.order_goods_qty}</h2>
				</td>
				<td>
							...
							<select name="s_delivery_state" id="s_delivery_state">
								<c:choose>
									...
									<c:when test="${deliveryInfo.delivery_state=='cancel_order' }">
										...
										<option value="returning_goods">반품</option>
									</c:when><c:when test="${deliveryInfo.delivery_state=='returning_goods' }">
										<option value="delivery_prepared">배송준비중</option>
										<option value="delivering">배송중</option>
										<option value="finished_delivering">배송완료</option>
										<option value="cancel_order">주문취소</option>
