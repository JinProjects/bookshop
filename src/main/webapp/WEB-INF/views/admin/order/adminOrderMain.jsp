...
function init(){
	...
	if(h_delivery_state.length==undefined){
		...
		for(var i=0; s_delivery_state.length;i++){
			s_delivery_state[i].value=h_delivery[i].value;
		}
	}
}
</script>
...
/* 서버에서 조작하는 것보다 자바스크립트에서 조작 후 
   서버로 결과값을 보내서 처리하는 것이 빠르다. 
*/
					...
					<td>
						<select name="curYear">
							<c:forEach var="i" begin="0" end="5">
								<!-- endYear-0이라서 같은 년도는 select 나머지는 목록 -->
								<c:choose>
				...
				<tr>
					<td>
						<select name="s_search_type" disabled>
							<option value="all" checked>전체</option><!-- checked 속성이 가능한지..? -->
