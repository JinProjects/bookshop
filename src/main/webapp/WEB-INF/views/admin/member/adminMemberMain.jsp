...
function search_member(search_period){
	alert("search_period="+search_period);
...
function calcPeriod(search_period){
	...
	endDay = dt.getDate();
	if(search_period=='today'){//당일
		beginYear=endYear;
		beginMonth=endMonth;
		beginDay=endDay;
	}else if(search_period=='one_week'){//1주 전(-7)
		beginYear=dt.getFullYear();
		if(endDay-7<1){//만일 endDay가 6 or 7이면 전 달로 바꿔야함
	...
	//월 또는 일이 10이하일 경우 앞에 0을 붙여주고 출력
	if(beginMonth <10){
			...
			<c:otherwise>
				<c:forEach var="item" items="${member_list }" varStatus="item_num">
					<tr>
						...
						<td width="10%">
							<strong>${item.hp1 }-${item.hp2 }-${item.hp3 }</strong>
						</td>
						<td width="50%">
				 ...
				 <td colspan="8" class="fixed">
				 	<c:forEach var="page" begin="1" end="10" step="1">
				 		<c:if test="${section>1 && page==1 }">
				 			<a href="${contextPath }/admin/member/adminMemberMain.do?section=${section-1}&pageNum=${(section-1)*10+1}">&nbsp;pre&nbsp;</a>
				 		</c:if>
