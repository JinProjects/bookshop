	...
	<c:otherwise>
		<script type="text/javascript">
			...
			function init(){
				...
				var h_tel1=frm_mod_member.h_tel1;
				var h_hp1=frm_mod_member.h_hp1;
				var tel1=h_tel1.value;//서버로부터 값을 받아와서 hidden태크에 저장하고 초기화
				var hp1=h_hp1.value;
...
<script type="text/javascript">
	function fn_modify_member_info(member_id,mod_type){
		...
		if(mod_type=='member_pw'){
			...
			var member_birth_y=frm_mod_member.birth_y;
			var member_birth_m=frm_mod_member.birth_m;
			var member_birth_d=frm_mod_member.birth_d;
			var member_birth_gn=frm_mod_member.birth_gn;
			...
			for(var i=0; member_birth_d.length;i++){
				if(member_birth_d[i].selected){
					value_d=member_birth_m[i].value;
					break;
				}
			}
		...
		$.ajax({
			type:"post",
			async:false,
			url:"http://localhost:8091/bookshop/admin/member/modifyMemberInfo.do",
			data:{
				member_id:member_id,
				mod_type:mod_type,
				value:value
...
function fn_delete_member(member_id,del_yn){
	...
	frm_mod_member.action="/bookshop/admin/member/deleteMember.do";
		...
		<table>
			<tbody>
				<tr class="dot_line">
					...
					<td>
						<input type="text" name="member_id" size="20" value="${member_info.member_id }" disabled>
					</td>
					<td>
				...
				<tr class="dot_line">
					...
					<td>
						<input type="password" name="member_pw" size="20" value="${member_info.member_pw }">
					</td>
					<td>
				...
				<tr class="dot_line">
					...
					<td>
						<input type="text" name="member_name" size="20" value="${member_info.member_name }" disabled>
					</td>
					<td>
						<input type="button" value="수정하기" onclick="fn_modify_member_info('${member_info.member_id}','member_name')" disabled>
					</td>
				...
				<tr class="dot_line">
					...
					<td>
						...
						<select name="select_email2" title="직접입력" onchange="">
				...
				<tr class="dot_line">
					...
					<td>
						...
						<p>
							...
							나머지 주소: <input type="text" id="namujiAddress" name="namujiAddress" size="50" value="${member_info.namujiAddress}"> <br> <br>
